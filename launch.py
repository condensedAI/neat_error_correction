...
# We just load it
if args.saveDir is not None and os.path.exists(args.saveDir):
    with open("%s/config.json"%args.saveDir, "r") as f:
        config = json.load(f)
...
game.run(savedir, args.numParallelJobs, args.verbose)

elapsed = datetime.datetime.now() - time_id
